...
def failed(message):
    if message == "":
        print
        "mkrelease.py failed to complete"
    else:
        print
        message
    sys.exit(2)
...
try:
    ...
    print
    "Usage: python mkrelease.py version branch username"
    print
    "where version should be x.y.z and username is your SF username"
    sys.exit(1)
...
print
"Now log in to SourceForge and set the operating systems applicable to the newly uploaded tarball and zip file. Also remember to do a 'git push --tags'."
