...
if string.lower(dirname) != dirname:
    print
    "directory name (" + dirname + ") should be in lowercase"
    sys.exit(3)
...
print
"Removing " + dirname + ".tar if exists"
os.system("rm -f " + dirname + ".tar.gz")

print
"Removing " + dirname + ".tar.gz if exists"
os.system("rm -f " + dirname + ".tar")

# Grab the code from git

print
"Checking git repository is in sync with remote repository"
os.system("git remote update origin") == 0 or failed()
command = ["git", "status", "--porcelain", "-uno"]
out = subprocess.check_output(command)
if out.strip() != "":
    print
    "Local git repository has modifications"
    print
    " ".join(command)
    print
    out
...
if out.strip() != "":
    print
    "Remote repository has additional modifications to local repository"
    print
    " ".join(command)
    print
    out
...
if out.strip() != "":
    print
    "Local repository has modifications not pushed to the remote repository"
    print
    "These should be pushed and checked that they pass Continuous Integration testing before continuing"
    print
    " ".join(command)
    print
    out
...
print
"Grabbing tagged release git repository using 'git archive' into " + outdir
...
print
"Finished building " + dirname + ".tar.gz"
